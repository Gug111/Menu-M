 ...
 const [form, setForm] = useState({
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const savedDeleted = localStorage.getItem('deletedDishes');
      if (saved) setDishes(JSON.parse(saved));
      else setDishes(defaultDishes);
      if (savedDeleted) setDeleted(JSON.parse(savedDeleted));
    }
  }, []);
  ...
  function addDish() {
    ...
    setForm({ name:'', nameGeo:'', description:'', category:'Hot / ცხელი', price:'', emoji:'🍽️', image: null });
...
function startEdit(dish) {
  ...
  function deleteDish(id) {
    ...
    localStorage.setItem('deletedDishes', JSON.stringify(updatedDeleted));
  }
  ...
  style={{ ...inputStyle, width: '54px', textAlign: 'center' }} />
          ...
          <div>
            ...
            <button onClick={() => fileInputRef.current.click()} style={{
              ...
              borderRadius: '10px', border: '0.5px dashed #B8DFC3',
    ...
    <img src={d.image} alt={d.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ...
                <button onClick={() => toggleDish(d.id)} style={{
                  ...
                  <div style={{
                    ...
                    top: '4px', left: d.available ? '20px' : '4px',
              ...
              <div style={{
                ...
                {/* Photo upload in edit — uses unique id per dish */}
                <div>
                  <input
                    type="file"
                    accept="image/*"
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={saveEdit} style={{
                    flex: 1, padding: '10px', background: '#1A3A2A',
                  ...
                  <button onClick={() => setEditingId(null)} style={{
                    ...
                    color: '#1A1A1A', border: '0.5px solid #E8E8E4', borderRadius: '10px',
                    fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit'
                  }}>
                    Cancel
                  </button>
